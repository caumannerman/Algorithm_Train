...
학생들은 복도의 빈 칸 중에서 장애물을 설치할 위치를 골라, 정확히 3개의 장애물을 설치해야 한다. 결과적으로 3개의 장애물을 설치하여 모든 학생들을 감시로부터 피하도록 할 수 있는지 계산하고자 한다.
 NxN 크기의 복도에서 학생 및 선생님의 위치 정보가 주어졌을 때, 장애물을 정확히 3개 설치하여 모든 학생들이 선생님들의 감시를 피하도록 할 수 있는지 출력하는 프로그램을 작성하시오.
"""
# 입력 - 첫째 줄에 자연수 N이 주어진다. (3 ≤ N ≤ 6) 둘째 줄에 N개의 줄에 걸쳐서 복도의 정보가 주어진다.
...
def endpoint_st(graph, x,y):

    for i in range(2):

        nx = x
        ny = y
        while nx >= 1 and nx <= n and ny >= 1 and ny <= n and graph[nx][ny] == 'X':
            nx += dx[i]
            ny += dy[i]
            if nx == n+1 or ny == n+1:
                break
        ...
        if (side1 == 'S' and side2 == 'T') or (side1 == 'T' and side2 == 'S'):
            return True
...
def graph_st_adjc(graph):
    for i in range(1, n+1):
        for j in range(1, n+1):
            if graph[i][j] == 'S':
                if graph[i-1][j] == 'T' or graph[i][j-1] == 'T':
                    return True
            if graph[i][j] == 'T':
                if graph[i-1][j] == 'S' or graph[i][j-1] == 'S':
                    return True
    return False
