...
이와 같이 장마철에 내리는 비의 양에 따라서 물에 잠기지 않는 안전한 영역의 개수는 다르게 된다. 위의 예와 같은 지역에서 내리는 비의 양에 따른 모든 경우를
다 조사해 보면 물에 잠기지 않는 안전한 영역의 개수 중에서 최대인 경우는 5임을 알 수 있다.
    ...
    while q:
        ...
        for i in range(4):
            ...
            if nx >= 0 and ny >= 0 and nx < n and ny < n:
                if data[nx][ny] > flag and not visited[nx][ny]:
                    visited[nx][ny] = True
                    q.append((nx, ny))
...
for i in range(1, maxi):
    ...
    for j in range(n):
        for k in range(n):
            if data[j][k] > i and not visited[j][k]:
                safe += 1
                bfs(j, k, i)
