...
""" 이 문제에서 BFS를 사용하지 못하는 이유: 단순하게 a->b로 갈 수 있는 지 판단하고싶으면 BFS로 더 빠르게 수행 가능.
but, 여기서는 start->end로 가는 최소 "비용"을 구해야한다. 
간선 가중치가 없다면, BFS로 deque에 차례대로 넣어가며, 가장 빨리 나오는 end노드 --> 이때의 비용을 가져오면 되지만, 
이 문제에서는 가중치가 있는 간선들이기 때문에, deque를 이용한다? ==> 결국 minimun_distance()함수가 필요 ==> 이게 다익스트라.
가중치가 있으면 BFS는 잊어버리자."""
import sys
import heapq
