...
# 입력 : 첫째 줄에 N(1 ≤ N ≤ 1,000), M(1 ≤ M ≤ 10,000), X가 공백으로 구분되어 입력된다.
...
def dijkstra(graph,distarr, start):
    distarr[start] = 0
    q = [(0,start)]
    while q:
...
dijkstra(graph_r, distance_r, x)
result = -1

for i in range(1, n+1):
    if distance[i] + distance_r[i] > result:
        result = distance[i] + distance_r[i]
print(result)