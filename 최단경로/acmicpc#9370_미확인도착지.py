...
예제 입력의 두 번째 케이스를 시각화한 것이다. 이 듀오는 회색 원에서 두 검은 원 중 하나로 가고 있고 점선으로 표시된 도로에서 냄새를 맡았다.
따라서 그들은 6으로 향하고 있다."""
# 입력 : 첫 번째 줄에는 테스트 케이스의 T(1 ≤ T ≤ 100)가 주어진다. 각 테스트 케이스마다
#
...
for _ in range(T):
    ...
    dg = dijkstra(graph, g)
    for i in mokhubo:
        if ds[h] + jina + dg[i] == ds[i] or ds[g] + jina + dh[i] == ds[i]:
            result.append(i)
    result.sort()
