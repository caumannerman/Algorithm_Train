...
# 입력 : 첫째 줄에 두 정수 L, C가 주어진다. (3 ≤ L ≤ C ≤ 15) 다음 줄에는 C개의 문자들이 공백으로 구분되어 주어진다.
...
data.sort()
result = ['a']*l
''' 모음 최소 1개, 자음 최소 2개의 조건을 매개변수로 처리해주었다.'''
def dfs(level,begin,mo,ja):
    if level == l:
        if mo >= 1 and ja>= 2:
            print("".join(result))
        return
    for i in range(begin,c):
        ...
        if data[i] in ('a','e','i','o','u'):
            dfs(level+1,i+1,mo+1,ja)
        else:
            dfs(level+1, i+1, mo, ja+1)
dfs(0,0,0,0)